...
const Login = () => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(result[0].defaultMessage || response.text || "Signup failed.");
      }
    } catch (error) {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen h-screen bg-gray-100 dark:bg-DarkPrimary-200">
      {error && <Alert severity="error" className="fixed top-2 left-2 right-2 z-10" onClose={() => {setError("")}}>{error}</Alert>}
      {success && !error && <Alert severity="success" className="fixed top-2 left-2 right-2 z-10">Login Successful!</Alert>}
